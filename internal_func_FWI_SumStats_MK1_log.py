...
This skript is optimized for the Franke Westerhoff I model.
"""
import numpy as np
#from sklearn.covariance import MinCovDet #as MinCovDet
import FrankWesterhoffI
#import proposal_SumStats_log
import SumStats_MK1_log
#import FWI_sumstats_short_log
...
def syn_loop(T,M,theta,funda,y):
    ...
    x = np.log(FrankWesterhoffI.FrankeWestI(T, theta, funda, y))
...
def log_prob(ssy, the_mu, the_cov):
    ...
    """
    return  -0.5*np.log(np.linalg.det(the_cov))-0.5*(ssy-the_mu)@np.linalg.inv(the_cov)@(ssy-the_mu).T
...
def ESS_theta_func(ssy,theta,funda,y,M,T):
    ...
    """
    s_para=Parallel(n_jobs=60)(delayed(syn_loop)(T,M,theta_0,funda,y) for _ in range(0,M))
    #changes s_para to a numpy matrix
    s = np.asarray(s_para)
    #the mean and the variance/covariance matrix are calculated
    the_mu = s.sum(axis=0)/M
